...
export class CapabilityRetriever {
    ...
    retrieve(intent, options) {
        ...
        for (const category of categories) {
            ...
            const normalizedMatches = retrieval.matches.map(match => normalizeCandidate(match.tool, match.score, match.strategy, category, query));
    ...
    safeSemanticRetrieval(query, options) {
        try {
    ...
    runDeterministicFallback(query, category, options) {
...
function normalizeCandidate(tool, score, strategy, matchedCategory, query) {
    return {
        tool,
        toolId: tool.id,
        toolName: tool.name,
