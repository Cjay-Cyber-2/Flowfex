...
export class TaskIntentPlanner {
    ...
    buildFallbackIntent(task, availableCategories) {
...
function detectCategories(task, availableCategories) {
...
function buildFallbackBranchPoints(task, steps) {
    if (steps.length < 2) {
        return [];
    }
...
function detectConstraints(task) {
    const constraints = [];
