...
export class FlowfexServer {
  ...
  async _handleRequest(request, response) {
    ...
    if (request.method === 'GET' && skillsMatch) {
      if (!await this._isSkillCatalogCallerAllowed(request, authUser, anonymousToken)) {
        return this._writeJson(response, 401, {
          error: {
  ...
  async _isSkillCatalogCallerAllowed(request, authUser, anonymousToken) {
    ...
    if (anonymousToken && this.anonymousSessionService) {
      try {
        const session = await this.anonymousSessionService.validateAnonymousSession(anonymousToken);
        if (session?.id) {
          return true;
        }
      } catch {
    ...
    // An active agent session token (Bearer ...) also unlocks read access so
    // SDK / live-channel agents can introspect the catalog they are attached
    // to.
    const bearer = this._extractBearerToken(request);
    if (bearer && this.connectionService?.sessionManager?.findSessionByToken) {
      const tokenSession = this.connectionService.sessionManager.findSessionByToken(bearer);
      if (tokenSession?.id) {
        return true;
      }
    }
