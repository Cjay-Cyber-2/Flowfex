...
export const codeSnippets = {
  prompt: `Attach this agent to Syniq for the full conversation.

Session ID: ${DEMO_SESSION_ID}
Session URL: ${SESSION_URL}

Contract:
1. Stay attached to this Syniq session for the entire conversation.
2. For every user request, route the task to Syniq before taking action.
3. Always ask Syniq for agent.md files, skills, tools, memory, and workflows before continuing.
4. Use every directly relevant Syniq resource returned for that request.
5. If Syniq has no directly relevant resource for the request, use none and say the capability is unavailable in Syniq.
6. Do not invent substitute tools, skills, workflows, or resources outside Syniq.
7. Send progress, approval waits, and the final result back through the same Syniq session.`,
...
# Stay attached for the full conversation.
# Send every request to Syniq first.
# Use only directly relevant Syniq resources.
# If Syniq has no relevant resource, do not invent one.
